/*
   iMouse Project
   a part of Computer Engineering Essential subject
   Design by GROUP NO.32
*/
...
Vector MPU6050::readNormalizeAccel(void)
{
  ...
  na.XAxis = (int16_t) ra.XAxis * rangePerDigit * 9.80665f;
  na.YAxis = (int16_t) ra.YAxis * rangePerDigit * 9.80665f;
  na.ZAxis = (int16_t) ra.ZAxis * rangePerDigit * 9.80665f;
...
// End define MPU6050 Library -----------------------------------

// Start Define Constant ----------------------------------------

#define API_SERVER String("http://192.168.43.164:3000")
#define WIFI_SSID "OMGArchLinux"
#define WIFI_PSK "1q2w3e4r5t"
#define MAX_DEG 10.0
#define TRIG_DEG 1.0
#define LEFT_BUTTON D2
#define RIGHT_BUTTON D1
//#define CALIB_BUTTON D2
//#define RANDOM_BUTTON D2
#define CLICK_DELAY 20

// End Define Constant ------------------------------------------

// Start declare global variables here --------------------------
...
inline void loopCallback() {
  normAccel = mpu.readNormalizeAccel();

  acVal.x = normAccel.XAxis - xShift;
  acVal.y = normAccel.YAxis - yShift;
  acVal.z = normAccel.ZAxis - zShift;

  Serial.print(" Xnorm = ");
  Serial.print(acVal.x);
  Serial.print(" | Ynorm = ");
  Serial.print(acVal.y);
  Serial.print(" | Znorm = ");
  Serial.println(acVal.z);
...
inline void tryLeftClick() {
  if (digitalRead(LEFT_BUTTON) && (millis() - lastClickTime >= CLICK_DELAY)) {
    postData.x = 0;
    postData.y = 0;
    postData.option = 3;
    post();
...
inline void tryRightClick() {
  if (digitalRead(RIGHT_BUTTON) && (millis() - lastClickTime >= CLICK_DELAY)) {
    postData.x = 0;
    postData.y = 0;
    postData.option = 4;
    post();
...
//inline void tryCalibrate() {
//  if (digitalRead(CALIB_BUTTON) && (millis() - lastClickTime >= CLICK_DELAY)) {
//    setBalance();
//    lastClickTime = millis();
//  }
//}
//
//inline void tryRandom() {
//  if (digitalRead(RANDOM_BUTTON) && (millis() - lastClickTime >= CLICK_DELAY)) {
//    postData.option = millis() % 3 + 2;
//    postData.x = millis() % 100;
//    postData.y = ((millis() % 100) * 31) % 100;
//    post();
//    lastClickTime = millis();
//  }
//}

inline void tryMoveCursor() {
  postData.x = 0;
  postData.y = 0;
  postData.option = 2;
...
inline void setBalance() {
  ...
  yShift = 0;
  zShift = 0;
  Serial.println("Balancing Sensor...");
  for (uint8_t i = 0; i < 10; i++) {
    normAccel = mpu.readNormalizeAccel();
    xShift += normAccel.XAxis;
    yShift += normAccel.YAxis;
    zShift += normAccel.ZAxis;
    delay(150);
  }
  xShift /= 10.0;
  yShift /= 10.0;
...
void post() {
  switch (postData.option) {
    case 1:
      Serial.println("This API is not supported");
      break;
    case 2:
      http.begin(API_SERVER + "/move-by");
      Serial.println("Posting move by API");
      http.addHeader("Content-Type", "application/x-www-form-urlencoded");
      httpCode = http.POST("x=" + String(postData.x) + "&y=" + String(postData.y));
      break;
    case 3:
      http.begin(API_SERVER + "/left-click");
      Serial.println("Posting Left Click API");
      http.addHeader("Content-Type", "application/x-www-form-urlencoded");
      httpCode = http.POST("");
      break;
    case 4:
      http.begin(API_SERVER + "/right-click");
      Serial.println("Posting Right Click API");
      http.addHeader("Content-Type", "application/x-www-form-urlencoded");
      httpCode = http.POST("");
      break;
    case 5:
      ...
      break;
    default:
      Serial.println("Http Request didn't requested!!");
      return;
  }
  if (httpCode > 0) {
